...
**先从 google 找，再用 `cve`、`服务名`、`版本` 等关键字在 `kali` 里搜**，比较准。因为搜索引擎的检索比较可以智能地匹配你的关键字，但是 kali 里就没这么智能，关键字不匹配则比较难搜出来，即使本来就集成的 exp。
...
### 3、Port 139、445
这两个端口起的 `netbios-ssn` 服务，用的是 `samba`，google 一下当前版本，找到几个可能用得上。
